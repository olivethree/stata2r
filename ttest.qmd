...
-   **Cohen's d Calculation**:
    -   Cohen's d is calculated as the difference between two means divided by the pooled standard deviation of the groups. The formula is: \[ d = rac{ar{X}\_1 - ar{X}\_2}{\sqrt{rac{(n_1-1)s_1^2 + (n_2-1)s_2^2}{n_1 + n_2 - 2}}} \]
...
For a Paired Samples t-test, the common effect sizes to report include Cohen’s d for paired samples and the Correlation coefficient (r).

### Cohen’s d for Paired Samples Calculation:

In paired samples, Cohen’s d is calculated using the mean difference \(\bar{X}_D\) and the standard deviation of the differences \(s_D\). The formula is:
...
The correlation coefficient \(r\) for paired samples can be calculated from the t-statistic and degrees of freedom using the formula:
